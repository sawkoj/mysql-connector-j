...
public class NonRegisteringDriver implements java.sql.Driver {
    ...
    private JdbcConnection getConnection(ConnectionUrl connectionUrl, Properties info) throws SQLException {
        ...
        String connectionRedirectProperty = connectionUrl.getConnectionUrlParser().getProperties().get(PropertyKey.enableRedirect.getKeyName());
        if (Objects.nonNull(redirectionDataCache.get(currentHost)) &&
            (RedirectionOption.ON.toString().equalsIgnoreCase(connectionRedirectProperty) ||
            RedirectionOption.PREFERRED.toString().equalsIgnoreCase(connectionRedirectProperty))) {
